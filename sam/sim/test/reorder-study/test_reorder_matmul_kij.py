...
def test_reorder_matmul_kij(samBench, sparsity, check_gold, debug_sim, backpressure, depth, fill=0):
    ...
    fiberlookup_Bi_15 = CompressedCrdRdScan(crd_arr=B_crd0, seg_arr=B_seg0, debug=debug_sim,
                                            back_en=backpressure, depth=int(depth))
    repsiggen_i_13 = RepeatSigGen(debug=debug_sim, back_en=backpressure, depth=int(depth))
    repeat_Ci_12 = Repeat(debug=debug_sim, back_en=backpressure, depth=int(depth))
