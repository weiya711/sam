...
def test_unit_mat_sddmm_coiter_fused(samBench, check_gold, debug_sim, fill=0):

    B_dirname = os.path.join(cwd, "tmp_mat_sddmm")
    ...
    D_vals_filename = os.path.join(B_dirname, "tensor_D_mode_vals")
    ...
    while not done and time_cnt < TIMEOUT:
        ...
        inti.append(intersecti_24.out_crd())
        ...
        repeat_Di_21.update()
        di.append(repeat_Di_21.out_ref())
        ...
        intj.append(intersectj_18.out_crd())
        ...
        intk.append(intersectk_12.out_crd())
    ...
    if debug_sim:
        print("inti", remove_emptystr(inti))
        print("di", remove_emptystr(di))
        print("intj", remove_emptystr(intj))
        print("cj", remove_emptystr(cj))
        print("ck", remove_emptystr(ck))
        print("intk", remove_emptystr(intk))
        print("bk", remove_emptystr(bk))
    ...
    if check_gold:
        ...
        c_mat = np.array(C_vals).reshape(C_shape)
        d_mat = np.array(D_vals).reshape(D_shape).transpose()
