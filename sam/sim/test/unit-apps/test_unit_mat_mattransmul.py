...
def test_unit_mat_mattransmul(samBench, ssname, check_gold, debug_sim, fill=0):
    C_dirname = os.path.join(cwd, "tmp_mat_mattransmul")
    ...
    dirname = C_dirname
    d_shape = C_shape[0]
    d0_seg_filename = os.path.join(dirname, "tensor_d_mode_0_seg")
    d_seg0 = read_inputs(C0_seg_filename)
    d0_crd_filename = os.path.join(dirname, "tensor_d_mode_0_crd")
    d_crd0 = read_inputs(d0_crd_filename)

    d_shape = C_shape[0]
    d_vals_filename = os.path.join(dirname, "tensor_d_mode_vals")
    ...
    e_shape = [1]
    e_dirname = C_dirname
    e_vals_filename = os.path.join(e_dirname, "tensor_e_mode_vals")
    e_vals = read_inputs(e_vals_filename, float)

    f_shape = C_shape[1]

    f0_seg_filename = os.path.join(dirname, "tensor_f_mode_0_seg")
    f_seg0 = read_inputs(C0_seg_filename)
    f0_crd_filename = os.path.join(dirname, "tensor_f_mode_0_crd")
    f_crd0 = read_inputs(f0_crd_filename)

    f_vals_filename = os.path.join(dirname, "tensor_f_mode_vals")
    ...
    while not done and time_cnt < TIMEOUT:
        ...
        dj.append(fiberlookup_dj_19.out_crd())
        cj.append(fiberlookup_Cj_18.out_crd())
        intj.append(intersectj_17.out_crd())
        rsgj.append(repsiggen_j_16.out_repsig())
        valb.append(arrayvals_b_6.out_val())
        valc.append(arrayvals_C_7.out_val())

        if debug_sim:
            print("dj", remove_emptystr(dj))
            print("cj", remove_emptystr(cj))
            print("intj", remove_emptystr(intj))
            print("rsgj", remove_emptystr(rsgj))
