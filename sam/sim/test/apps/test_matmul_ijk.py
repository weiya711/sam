...
def test_matmul_ijk(ssname, debug_sim, fill = 0):
    ...
    fiberwrite_X1 = CompressWrScan(seg_size = B_shape[0] + 1, size=B_shape[0] * C_shape[0], fill = fill, debug = debug_sim)
    fiberlookup_Ck = CompressedRdScan(crd_arr=C_crd0, seg_arr=C_seg0, debug=debug_sim)
    ...
    fiberwrite_Xvals = ValsWrScan(size= 1 * B_shape[0] * C_shape[0], fill=fill, debug=debug_sim)
