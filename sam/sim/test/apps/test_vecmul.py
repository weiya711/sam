...
@pytest.mark.vec
def test_vecmul(samBench, vecname, check_gold, debug_sim, report_stats, fill=0):
    B_dirname = os.path.join(formatted_dir, vecname, "orig", "ds01")
    ...
    fiberlookup_Bi_12 = UncompressCrdRdScan(dim=B_shape[0], debug=debug_sim, statistics=report_stats)
    fiberlookup_Bj_7 = CompressedCrdRdScan(crd_arr=B_crd1, seg_arr=B_seg1, debug=debug_sim, statistics=report_stats)
    fiberwrite_x0_0 = CompressWrScan(seg_size=2, size=B_shape[0], fill=fill, debug=debug_sim, statistics=report_stats)
    ...
    fiberwrite_xvals_1 = ValsWrScan(size=B_shape[0], fill=fill, debug=debug_sim, statistics=report_stats)
    in_ref_B = [0, 'D']
    in_ref_c = [0, 'D']
    done = False
    time_cnt = 0
