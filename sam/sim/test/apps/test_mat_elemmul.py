...
def test_mat_elemmul(samBench, ssname, check_gold, debug_sim, report_stats, fill=0):
    ...
    fiberlookup_Ci_12 = CompressedCrdRdScan(crd_arr=C_crd0, seg_arr=C_seg0, debug=debug_sim, statistics=report_stats)
    intersecti_10 = Intersect2(debug=debug_sim, statistics=report_stats)
    fiberlookup_Bj_8 = CompressedCrdRdScan(crd_arr=B_crd1, seg_arr=B_seg1, debug=debug_sim, statistics=report_stats)
    fiberlookup_Cj_9 = CompressedCrdRdScan(crd_arr=C_crd1, seg_arr=C_seg1, debug=debug_sim, statistics=report_stats)
    intersectj_7 = Intersect2(debug=debug_sim, statistics=report_stats)
    crddrop_6 = CrdDrop(debug=debug_sim, statistics=report_stats)
    ...
    fiberwrite_X1_1 = CompressWrScan(seg_size=B_shape[0] + 1, size=B_shape[0] * B_shape[1], fill=fill, debug=debug_sim, statistics=report_stats)
    mul_3 = Multiply2(debug=debug_sim, statistics=report_stats)
    fiberwrite_Xvals_0 = ValsWrScan(size=1 * B_shape[0] * B_shape[1], fill=fill, debug=debug_sim, statistics=report_stats)
    in_ref_B = [0, 'D']
    in_ref_C = [0, 'D']
    done = False
    time_cnt = 0
    ...
    for k in sample_dict.keys():
        extra_info["fiberlookup_Bj_8" + "_" + k] = sample_dict[k]
    ...
    sample_dict = fiberlookup_Cj_9.return_statistics()
    for k in sample_dict.keys():
        extra_info["fiberlookup_Cj_9" + "_" + k] = sample_dict[k]

    sample_dict = fiberlookup_Ci_12.return_statistics()
    for k in sample_dict.keys():
        extra_info["fiberlookup_Ci_12" + "_" + k] = sample_dict[k]
