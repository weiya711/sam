...
def test_tensor4_mul(samBench, frosttname, check_gold, debug_sim, report_stats, fill=0):
    ...
    C_dirname = os.path.join(formatted_dir, frosttname + "_dense", "other", "ssss0123")
    C_vals_filename = os.path.join(C_dirname, "B_vals.txt")
    C_ref_dense = read_inputs(C_vals_filename)
    C_ref = torch.Tensor(C_ref_dense)
    C_ref = C_ref.view(C_shape)

    gold_ref = torch.einsum('ijkl, ijlm->ijkm', B_ref, C_ref)
    print(gold_ref)
    ...
    fiberwrite_X0_4 = CompressWrScan(seg_size=2, size=2 * len(B_crd0), fill=fill, debug=debug_sim, statistics=report_stats)
    ...
    fiberwrite_X1_3 = CompressWrScan(seg_size=2 * len(B_crd0) + 1, size=2 * len(B_crd1), fill=fill,
    ...
    fiberwrite_X2_2 = CompressWrScan(seg_size=2 * len(B_crd1) + 1, size=2 * len(B_crd2), fill=fill,
                                     debug=debug_sim, statistics=report_stats)
    ...
    # fiberwrite_X3_1 = CompressWrScan(seg_size=B_shape[0] * B_shape[1] * B_shape[0] + 1, size=B_shape[0] * B_shape[1] * B_shape[0] * C_shape[1], fill=fill, debug=debug_sim, statistics=report_stats)
    fiberwrite_X3_1 = CompressWrScan(seg_size=2 * len(B_crd2) + 1, size=len(B_vals) * 2, fill=fill,
                                     debug=debug_sim, statistics=report_stats)
    ...
    reduce_26 = Reduce(debug=debug_sim, statistics=report_stats)
    # fiberwrite_Xvals_0 = ValsWrScan(size=1 * B_shape[0] * B_shape[1] * B_shape[0] * C_shape[1], fill=fill, debug=debug_sim, statistics=report_stats)
    ...
    while not done and time_cnt < TIMEOUT:
        ...
        fiberlookup_Bk_19.set_in_ref(intersectj_20.out_ref1())
        fiberwrite_X2_2.set_input(fiberlookup_Bk_19.out_crd())
        ...
        fiberlookup_Bl_15.set_in_ref(fiberlookup_Bk_19.out_ref())
        ...
        reduce_26.set_in_val(reduce_5.out_val())
        fiberwrite_Xvals_0.set_input(reduce_26.out_val())
        test.append(mul_6.out_val())
