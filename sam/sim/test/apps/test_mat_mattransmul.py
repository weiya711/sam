...
def test_mat_mattransmul(samBench, ssname, check_gold, debug_sim, report_stats, fill=0):
    ...
    unioni_26 = Union2(debug=debug_sim, statistics=report_stats)
    fiberlookup_Cj_18 = CompressedCrdRdScan(crd_arr=C_crd0, seg_arr=C_seg0, debug=debug_sim, statistics=report_stats)
    fiberwrite_x0_1 = CompressWrScan(seg_size=2, size=C_shape[1], fill=fill, debug=debug_sim, statistics=report_stats)
    repsiggen_i_24 = RepeatSigGen(debug=debug_sim, statistics=report_stats)
    repeat_bi_20 = Repeat(debug=debug_sim, statistics=report_stats)
    repeat_di_21 = Repeat(debug=debug_sim, statistics=report_stats)
    repeat_ei_22 = Repeat(debug=debug_sim, statistics=report_stats)
    fiberlookup_dj_19 = CompressedCrdRdScan(crd_arr=d_crd0, seg_arr=d_seg0, debug=debug_sim, statistics=report_stats)
    intersectj_17 = Intersect2(debug=debug_sim, statistics=report_stats)
    ...
    repeat_ej_13 = Repeat(debug=debug_sim, statistics=report_stats)
    repeat_fj_14 = Repeat(debug=debug_sim, statistics=report_stats)
    ...
    mul_9 = Multiply2(debug=debug_sim, statistics=report_stats)
    ...
    fiberwrite_xvals_0 = ValsWrScan(size=1 * C_shape[1], fill=fill, debug=debug_sim, statistics=report_stats)
    in_ref_C = [0, 'D']
    in_ref_f = [0, 'D']
    in_ref_b = [0, 'D']
    in_ref_d = [0, 'D']
