...
def test_matmul_kji(samBench, ssname, check_gold, debug_sim, report_stats, fill=0):
    ...
    fiberlookup_Cj_15 = CompressedCrdRdScan(crd_arr=C_crd1, seg_arr=C_seg1, debug=debug_sim, statistics=report_stats)
    repsiggen_j_13 = RepeatSigGen(debug=debug_sim, statistics=report_stats)
    repeat_Bj_12 = Repeat(debug=debug_sim, statistics=report_stats)
    fiberlookup_Bi_11 = CompressedCrdRdScan(crd_arr=B_crd0, seg_arr=B_seg0, debug=debug_sim, statistics=report_stats)
    ...
    repsiggen_i_9 = RepeatSigGen(debug=debug_sim, statistics=report_stats)
    repeat_Ci_8 = Repeat(debug=debug_sim, statistics=report_stats)
    ...
    sample_dict = fiberlookup_Bi_11.return_statistics()
    for k in sample_dict.keys():
        extra_info["fiberlookup_Bi_11" + "_" + k] = sample_dict[k]
    ...
    sample_dict = fiberlookup_Cj_15.return_statistics()
    for k in sample_dict.keys():
        extra_info["fiberlookup_Cj_15" + "_" + k] = sample_dict[k]
