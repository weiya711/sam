...
def test_matmul_jki(samBench, ssname, check_gold, backpressure, depth, debug_sim, fill=0):
    ...
    fiberlookup_Ck_15 = CompressedCrdRdScan(crd_arr=C_crd0, seg_arr=C_seg0, debug=debug_sim,
                                            back_en=backpressure, depth=depth)
    repsiggen_j_17 = RepeatSigGen(debug=debug_sim, back_en=backpressure, depth=depth)
    repeat_Bj_16 = Repeat(debug=debug_sim, back_en=backpressure, depth=depth)
    fiberlookup_Bk_14 = CompressedCrdRdScan(crd_arr=B_crd1, seg_arr=B_seg1, debug=debug_sim,
                                            back_en=backpressure, depth=depth)
    intersectk_13 = Intersect2(debug=debug_sim, back_en=backpressure, depth=depth)
    crdhold_5 = CrdHold(debug=debug_sim, back_en=backpressure, depth=depth)
    fiberlookup_Bi_12 = CompressedCrdRdScan(crd_arr=B_crd0, seg_arr=B_seg0, debug=debug_sim,
                                            back_en=backpressure, depth=depth)
    arrayvals_B_7 = Array(init_arr=B_vals, debug=debug_sim, back_en=backpressure, depth=depth)
    crdhold_4 = CrdHold(debug=debug_sim, back_en=backpressure, depth=depth)
    repsiggen_i_10 = RepeatSigGen(debug=debug_sim, back_en=backpressure, depth=depth)
    repeat_Ci_9 = Repeat(debug=debug_sim, back_en=backpressure, depth=depth)
    arrayvals_C_8 = Array(init_arr=C_vals, debug=debug_sim, back_en=backpressure, depth=depth)
    mul_6 = Multiply2(debug=debug_sim, back_en=backpressure, depth=depth)
    spaccumulator1_3 = SparseAccumulator1(debug=debug_sim, back_en=backpressure, depth=depth)
    spaccumulator1_3_drop_crd_inner = StknDrop(debug=debug_sim, back_en=backpressure, depth=depth)
    spaccumulator1_3_drop_crd_outer = StknDrop(debug=debug_sim, back_en=backpressure, depth=depth)
    ...
    fiberwrite_Xvals_0 = ValsWrScan(size=1 * C_shape[1] * B_shape[0], fill=fill, debug=debug_sim,
                                    back_en=backpressure, depth=depth)
    fiberwrite_X0_1 = CompressWrScan(seg_size=C_shape[1] + 1, size=C_shape[1] * B_shape[0], fill=fill,
    ...
    while not done and time_cnt < TIMEOUT:
        ...
        intersectk_13.set_in2(fiberlookup_Ck_15.out_ref(), fiberlookup_Ck_15.out_crd(), fiberlookup_Ck_15)
