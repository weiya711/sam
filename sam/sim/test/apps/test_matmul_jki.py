...
def test_matmul_jki(samBench, ssname, check_gold, debug_sim, report_stats, fill=0):
    ...
    fiberlookup_Cj_19 = CompressedCrdRdScan(crd_arr=C_crd1, seg_arr=C_seg1, debug=debug_sim, statistics=report_stats)
    fiberlookup_Ck_15 = CompressedCrdRdScan(crd_arr=C_crd0, seg_arr=C_seg0, debug=debug_sim, statistics=report_stats)
    repsiggen_j_17 = RepeatSigGen(debug=debug_sim, statistics=report_stats)
    repeat_Bj_16 = Repeat(debug=debug_sim, statistics=report_stats)
    ...
    fiberwrite_Xvals_0 = ValsWrScan(size=1 * C_shape[1] * B_shape[0], fill=fill, debug=debug_sim, statistics=report_stats)
    fiberwrite_X0_1 = CompressWrScan(seg_size=C_shape[1] + 1, size=C_shape[1] * B_shape[0], fill=fill, debug=debug_sim, statistics=report_stats)
    fiberwrite_X1_2 = CompressWrScan(seg_size=2, size=C_shape[1], fill=fill, debug=debug_sim, statistics=report_stats)
    in_ref_C = [0, 'D']
    in_ref_B = [0, 'D']
    done = False
    time_cnt = 0
    ...
    while not done and time_cnt < TIMEOUT:
        ...
        fiberlookup_Ck_15.set_in_ref(fiberlookup_Cj_19.out_ref())
        repsiggen_j_17.set_istream(fiberlookup_Cj_19.out_crd())
        if len(in_ref_B) > 0:
            repeat_Bj_16.set_in_ref(in_ref_B.pop(0))
        repeat_Bj_16.set_in_repsig(repsiggen_j_17.out_repsig())
        fiberlookup_Bk_14.set_in_ref(repeat_Bj_16.out_ref())
        intersectk_13.set_in1(fiberlookup_Bk_14.out_ref(), fiberlookup_Bk_14.out_crd())
        intersectk_13.set_in2(fiberlookup_Ck_15.out_ref(), fiberlookup_Ck_15.out_crd())
        crdhold_5.set_outer_crd(fiberlookup_Cj_19.out_crd())
        crdhold_5.set_inner_crd(intersectk_13.out_crd())
        fiberlookup_Bi_12.set_in_ref(intersectk_13.out_ref1())
        arrayvals_B_7.set_load(fiberlookup_Bi_12.out_ref())
        crdhold_4.set_outer_crd(crdhold_5.out_crd_outer())
        crdhold_4.set_inner_crd(fiberlookup_Bi_12.out_crd())
        repsiggen_i_10.set_istream(fiberlookup_Bi_12.out_crd())
        repeat_Ci_9.set_in_ref(intersectk_13.out_ref2())
        repeat_Ci_9.set_in_repsig(repsiggen_i_10.out_repsig())
        arrayvals_C_8.set_load(repeat_Ci_9.out_ref())
        mul_6.set_in1(arrayvals_C_8.out_val())
        mul_6.set_in2(arrayvals_B_7.out_val())
        spaccumulator1_3_drop_crd_inner.set_in_stream(crdhold_5.out_crd_inner())
    ...
    sample_dict = fiberlookup_Cj_19.return_statistics()
    for k in sample_dict.keys():
        extra_info["fiberlookup_Cj_19" + "_" + k] = sample_dict[k]
