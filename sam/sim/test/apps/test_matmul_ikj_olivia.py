...
def test_matmul_ikj(samBench, ssname, check_gold, debug_sim, backpressure, depth, fill=0):
    ...
    fiberwrite_X0_2 = CompressWrScan(seg_size=2, size=B_shape[0], fill=fill, debug=debug_sim,
                                     back_en=backpressure, depth=depth)
    repsiggen_i_15 = RepeatSigGen(debug=debug_sim, back_en=backpressure, depth=depth)
    repeat_Ci_14 = Repeat(debug=debug_sim, back_en=backpressure, depth=depth)
    fiberlookup_Ck_13 = CompressedCrdRdScan(crd_arr=C_crd0, seg_arr=C_seg0, debug=debug_sim,
                                            back_en=backpressure, depth=depth)
    intersectk_11 = Intersect2(debug=debug_sim, back_en=backpressure, depth=depth)
    fiberlookup_Cj_10 = CompressedCrdRdScan(crd_arr=C_crd1, seg_arr=C_seg1, debug=debug_sim,
                                            back_en=backpressure, depth=depth)
    arrayvals_C_6 = Array(init_arr=C_vals, debug=debug_sim, back_en=backpressure, depth=depth)
    repsiggen_j_8 = RepeatSigGen(debug=debug_sim, back_en=backpressure, depth=depth)
    repeat_Bj_7 = Repeat(debug=debug_sim, back_en=backpressure, depth=depth)
    arrayvals_B_5 = Array(init_arr=B_vals, debug=debug_sim, back_en=backpressure, depth=depth)
    mul_4 = Multiply2(debug=debug_sim, back_en=backpressure, depth=depth)
    spaccumulator1_3 = SparseAccumulator1(debug=debug_sim, back_en=backpressure, depth=depth)
    spaccumulator1_3_drop_crd_in_inner = StknDrop(debug=debug_sim, back_en=backpressure, depth=depth)
    spaccumulator1_3_drop_crd_in_outer = StknDrop(debug=debug_sim, back_en=backpressure, depth=depth)

    spaccumulator1_3_crd_hold_in_ik = CrdHold(debug=debug_sim, back_en=backpressure, depth=depth)
    spaccumulator1_3_crd_hold_in_ij = CrdHold(debug=debug_sim, back_en=backpressure, depth=depth)

    spaccumulator1_3_drop_val = StknDrop(debug=debug_sim, back_en=backpressure, depth=depth)
    ...
    fiberwrite_X1_1 = CompressWrScan(seg_size=B_shape[0] + 1, size=B_shape[0] * C_shape[1], fill=fill,
                                     debug=debug_sim, back_en=backpressure, depth=depth)
    in_ref_B = [0, 'D']
    in_ref_C = [0, 'D']
    done = False
    time_cnt = 0
    ...
    while not done and time_cnt < TIMEOUT:
        ...
        fiberlookup_Bk_12.set_in_ref(fiberlookup_Bi_17.out_ref(), fiberlookup_Bi_17)
        ...
        fiberlookup_Ck_13.set_in_ref(repeat_Ci_14.out_ref(), repeat_Ci_14)

        intersectk_11.set_in1(fiberlookup_Ck_13.out_ref(), fiberlookup_Ck_13.out_crd(), fiberlookup_Ck_13)
        intersectk_11.set_in2(fiberlookup_Bk_12.out_ref(), fiberlookup_Bk_12.out_crd(), fiberlookup_Bk_12)

        fiberlookup_Cj_10.set_in_ref(intersectk_11.out_ref1(), intersectk_11)

        arrayvals_C_6.set_load(fiberlookup_Cj_10.out_ref(), fiberlookup_Cj_10)

        repsiggen_j_8.set_istream(fiberlookup_Cj_10.out_crd(), fiberlookup_Cj_10)

        repeat_Bj_7.set_in_ref(intersectk_11.out_ref2(), intersectk_11)
        repeat_Bj_7.set_in_repsig(repsiggen_j_8.out_repsig(), repsiggen_j_8)
        ...
        spaccumulator1_3.crd_in_inner(spaccumulator1_3_drop_crd_in_inner.out_val(), spaccumulator1_3_drop_crd_in_inner)
