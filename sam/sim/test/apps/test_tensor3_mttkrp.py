...
@pytest.mark.frostt
def test_tensor3_mttkrp(samBench, frosttname, check_gold, debug_sim, report_stats, fill=0):
    ...
    fiberwrite_X1_1 = CompressWrScan(seg_size=B_shape[0] + 1, size=B_shape[0] * C_shape[0], fill=fill, debug=debug_sim, statistics=report_stats)
    ...
    fiberwrite_Xvals_0 = ValsWrScan(size=1 * B_shape[0] * C_shape[0], fill=fill, debug=debug_sim, statistics=report_stats)
    in_ref_B = [0, 'D']
    in_ref_C = [0, 'D']
    in_ref_D = [0, 'D']
    done = False
    ...
    for k in sample_dict.keys():
        extra_info["fiberlookup_Bi_31" + "_" + k] = sample_dict[k]
    ...
    for k in sample_dict.keys():
        extra_info["fiberlookup_Cj_24" + "_" + k] = sample_dict[k]
    ...
    for k in sample_dict.keys():
        extra_info["fiberlookup_Dl_14" + "_" + k] = sample_dict[k]
    ...
    for k in sample_dict.keys():
        extra_info["fiberlookup_Dj_25" + "_" + k] = sample_dict[k]
