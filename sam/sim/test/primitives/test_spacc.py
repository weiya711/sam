...
def test_SparseAccumulator1new_direct(arrs, debug_sim):
    ...
    sa = SparseAccumulator1(valtype=int, val_stkn=True, debug=debug_sim)
...
def test_SparseAccumulator1new_rand(dim, debug_sim, max_val=1000, fill=0):
    ...
    while not done and time < TIMEOUT:
        ...
        print("Timestep", time, "\t Done --",
              ...
              "\tSparseAccumulator1New:", sa.out_done(),
              "\tArr:", val_B.out_done(),
...
def test_SparseAccumulator2new_stream_direct(arrs, debug_sim):
    ...
    sa = SparseAccumulator2(valtype=int, val_stkn=True, debug=debug_sim)
...
@pytest.mark.parametrize("arrs", [arrs_dict1])
def test_SparseAccumulator2new_tensor_direct(arrs, debug_sim, fill=0):
...
def test_SparseAccumulator2new_tensor_rand(dim, debug_sim, max_val=1000, fill=0):
