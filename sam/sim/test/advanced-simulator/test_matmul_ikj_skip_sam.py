...
def test_matmul_ikj_tiled_lp(samBench, ssname, check_gold, debug_sim, report_stats, skip_empty, yaml_name, nbuffer, fill=0):
    ...
    #struct = {"i00": 1, "k00": 2, "j00": 2, "i0": 1, "k0": 3, "j0": 4}
    ...
    glb_model_b = memory_block(name="GLB_B", skip_blocks = skip_empty, nbuffer = nbuffer, element_size = memory_config["Bytes_per_element"], size=memory_config["Glb_memory"], bandwidth=memory_config["Glb_tile_bandwidth"], latency=memory_config["Global_Glb_latency"],  debug = debug_sim)
    glb_model_c = memory_block(name="GLB_C", skip_blocks = skip_empty, nbuffer = nbuffer, element_size = memory_config["Bytes_per_element"], size=memory_config["Glb_memory"], bandwidth=memory_config["Glb_tile_bandwidth"], latency=memory_config["Global_Glb_latency"], debug = debug_sim)
    ...
    while not done and time_cnt < TIMEOUT:
        ...
        if True: #not skip_empty:i isinstance(repeat_Bj00.out_ref(), int) and  isinstance(fiberlookup_Cj00.out_ref(), int)
            
            #print("B: ", repeat_Bj00.out_ref() , struct["k00"])
            #print("C: ", fiberlookup_Cj00.out_ref(), struct["j00"])
            ...
            if len(glb_b_fifo) > 0 and len(glb_c_fifo) > 0 and isinstance(glb_b_fifo[0], int) and isinstance(glb_c_fifo[0], int) and (glb_b_fifo[0]//struct["k00"] ,  glb_b_fifo[0]%struct["k00"]) in sizes_dict_level0["B"] and (glb_c_fifo[0]//struct["j00"] ,  glb_c_fifo[0]%struct["j00"]) in sizes_dict_level0["C"]:
                ...
                glb_b_fifo.pop(0)
                glb_c_fifo.pop(0)
            elif len(glb_b_fifo) > 0 and len(glb_c_fifo) > 0 and (not isinstance(glb_b_fifo[0], int) or not isinstance(glb_c_fifo[0], int)):
            ...
            if True: #not skip_empty:
                ...
                if len(mem_b_fifo) > 0 and len(mem_c_fifo) > 0:
                    if isinstance(mem_b_fifo[0], int) and isinstance(mem_c_fifo[0], int):
                        ...
                        if (B_i00_new, B_k00_new, B_i0_new, B_k0_new) in sizes_dict_level1["B"] and (C_k00_new, C_j00_new, C_k0_new, C_j0_new) in sizes_dict_level1["C"]:
                            mem_model_b.add_tile(mem_b_fifo[0]%10**4, sizes_dict_level1["B"][(B_i00_new, B_k00_new, B_i0_new, B_k0_new)], glb_B_token)
                            mem_model_c.add_tile(mem_c_fifo[0]%10**4, sizes_dict_level1["C"][(C_k00_new, C_j00_new, C_k0_new, C_j0_new)], glb_C_token)
                    else:
                        #print(mem_b_fifo[0], mem_c_fifo[0])
                        b_t = mem_b_fifo[0].split("_")
                        c_t = mem_c_fifo[0].split("_")

                        #print(mem_b_fifo[0], b_t)
                        #print(mem_c_fifo[0], c_t)
        ...
        if mem_model_b.valid_tile() and mem_model_c.valid_tile(): 
            ...
            if nbuffer:
                #print("token-b", mem_model_b.token())
            ...
            if nbuffer:
                #print("token-c", mem_model_c.token())
