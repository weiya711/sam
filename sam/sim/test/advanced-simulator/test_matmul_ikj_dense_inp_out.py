...
def test_matmul_ikj_tiled_lp(samBench, ssname, check_gold, debug_sim, report_stats, skip_empty, yaml_name, nbuffer, fill=0):
    ...
    while not done and time_cnt < TIMEOUT:
        ...
        if True:  # not skip_empty:
            if isinstance(repeat_Bj00.out_ref(), int) and\
                    (repeat_Bj00.out_ref() // struct["k00"], repeat_Bj00.out_ref() % struct["k00"]) in sizes_dict_level0["B"]:
                glb_model_b.add_tile(repeat_Bj00.out_ref(),
                                     sizes_dict_level0["B"][(repeat_Bj00.out_ref() // struct["k00"],
                                                            repeat_Bj00.out_ref() % struct["k00"])])
            else:
                glb_model_b.add_tile(repeat_Bj00.out_ref(), 8)
            if isinstance(fiberlookup_Cj00.out_ref(), int) and\
                    (fiberlookup_Cj00.out_ref() // struct["j00"],
                     fiberlookup_Cj00.out_ref() % struct["j00"]) in sizes_dict_level0["C"]:
                glb_model_c.add_tile(fiberlookup_Cj00.out_ref(),
                                     sizes_dict_level0["C"][(fiberlookup_Cj00.out_ref() // struct["j00"],
                                                            fiberlookup_Cj00.out_ref() % struct["j00"])])
            else:
        ...
        if flag_glb:
            ...
            if True:  # not skip_empty:
                if isinstance(repeat_Bj0.out_ref(), int) and (B_i00_, B_k00_,
                                                              repeat_Bj0.out_ref() // struct["k0"],
                                                              repeat_Bj0.out_ref() % struct["k0"]) in sizes_dict_level1["B"]:
                    mem_model_b.add_tile(repeat_Bj0.out_ref(),
                                         sizes_dict_level1["B"][(B_i00_, B_k00_, repeat_Bj0.out_ref() // struct["k0"],
                                                                 repeat_Bj0.out_ref() % struct["k0"])],
                                         glb_model_b.token())
                else:
                    mem_model_b.add_tile(repeat_Bj0.out_ref(), 8, glb_model_b.token())
                if isinstance(fiberlookup_Cj0.out_ref(), int) and\
                        (C_k00_, C_j00_, fiberlookup_Cj0.out_ref() // struct["j0"],
                         fiberlookup_Cj0.out_ref() % struct["j0"]) in sizes_dict_level1["C"]:
                    mem_model_c.add_tile(fiberlookup_Cj0.out_ref(),
                                         sizes_dict_level1["C"][(C_k00_, C_j00_, fiberlookup_Cj0.out_ref() // struct["j0"],
                                                                fiberlookup_Cj0.out_ref() % struct["j0"])],
                                         glb_model_c.token())
                else:
        ...
        if mem_model_b.valid_tile() and mem_model_c.valid_tile()\
                and not mem_model_b.out_done() and not mem_model_c.out_done():
            flag = True
            flag_token_mem = True
            tiled_done_processed = False
            in_ref_B = [0, 'D']
            ...
            fiberwrite_Xvals_0 = ValsWrScan(size=1 * B_shape[0] * C_shape[1],
        ...
        if flag:
            ...
            mem_model_x.update(tiled_done)
            glb_model_x.update(tiled_done)
            if tiled_skip:
                tiled_done = True
                ...
                if check_gold:
                    if not tiled_skip:
                        check_gold_matmul_tiled([B_i00, B_k00, B_i0, B_k0],
                                                [C_k00, C_j00, C_k0, C_j0], None, debug_sim,
                                                out_crds=out_crds, out_segs=out_segs,
