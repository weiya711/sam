...
def test_tiling(samBench, ssname, check_gold, report_stats, yaml_name, debug_sim,
                nbuffer, backpressure, depth, reorder_not_ideal, reorder_block_len, split_factor, fill=0):
    skip_empty = False
    flag = False
    ...
    while not done and time_cnt < TIMEOUT:
        ...
        if ref_k.get_token() != "":
            ...
            if True:  # not is_stkn(crd_k.get_token()):
                ...
                if isinstance(crd_k.get_token(), int):
                    temp_count += 1
            if crd_k.get_token() == "S1" or crd_k.get_token() == "S2" or\
                    crd_k.get_token() == "S3" or crd_k.get_token() == "S4":
        ...
        done = fiberwrite_X0.done and fiberwrite_X1.done and fiberwrite_X0.done and\
            fiberwrite_X1.done and fiberwrite_X2.done and fiberwrite_X3.done
        done2 = fiberwrite_Y0.done and fiberwrite_Y1.done and fiberwrite_Y0.done and\
            fiberwrite_Y1.done and fiberwrite_Y2.done and fiberwrite_Y3.done
        done = done and done2
        time_cnt += 1
        if debug_sim:
    ...
    if False:
        print(fiberwrite_X0.get_arr(), fiberwrite_X1.get_arr(),
              fiberwrite_X2.get_arr(), fiberwrite_X3.get_arr(), fiberwrite_Xvals.get_arr())
        print(fiberwrite_Y0.get_arr(), fiberwrite_Y1.get_arr(),
              fiberwrite_Y2.get_arr(), fiberwrite_Y3.get_arr(), fiberwrite_Yvals.get_arr())
        print(fiberwrite_X0.get_seg_arr(), fiberwrite_X1.get_seg_arr(),
              fiberwrite_X2.get_seg_arr(), fiberwrite_X3.get_seg_arr())
        print(fiberwrite_Y0.get_seg_arr(), fiberwrite_Y1.get_seg_arr(),
              fiberwrite_Y2.get_seg_arr(), fiberwrite_Y3.get_seg_arr())
    dict_x = {}
    dict_y = {}
    ...
    for i in range(len(fiberwrite_X2.get_seg_arr()) - 1):
        size = fiberwrite_X3.get_seg_arr()[fiberwrite_X2.get_seg_arr()[i + 1]] -\
            fiberwrite_X3.get_seg_arr()[fiberwrite_X2.get_seg_arr()[i]]
        dict_x[i] = size
    ...
    for j in range(len(fiberwrite_Y2.get_seg_arr()) - 1):
        size = fiberwrite_Y3.get_seg_arr()[fiberwrite_Y2.get_seg_arr()[j + 1]] -\
            fiberwrite_Y3.get_seg_arr()[fiberwrite_Y2.get_seg_arr()[j]]
        dict_y[j] = size
    ...
    fiberlookup_Bi00 = CompressedCrdRdScan(crd_arr=fiberwrite_X0.get_arr(),
                                           seg_arr=fiberwrite_X0.get_seg_arr(), debug=debug_sim)
    fiberlookup_Bk00 = CompressedCrdRdScan(crd_arr=fiberwrite_X1.get_arr(),
                                           seg_arr=fiberwrite_X1.get_seg_arr(), debug=debug_sim)
    ...
    fiberlookup_Ck00 = CompressedCrdRdScan(crd_arr=fiberwrite_Y0.get_arr(),
                                           seg_arr=fiberwrite_Y0.get_seg_arr(), debug=debug_sim)
    intersect_00 = Intersect2(debug=debug_sim)
    fiberlookup_Cj00 = CompressedCrdRdScan(crd_arr=fiberwrite_Y1.get_arr(),
                                           seg_arr=fiberwrite_Y1.get_seg_arr(), debug=debug_sim)
