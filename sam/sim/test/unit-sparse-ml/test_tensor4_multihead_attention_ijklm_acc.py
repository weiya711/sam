...
def test_tensor4_multihead_attention_ijklm(samBench, frosttname, cast, check_gold, debug_sim, backpressure, depth, report_stats, fill=0):
    ...
    crddrop_softmax0 = CrdDrop(debug=debug_sim, statistics=report_stats, back_en=backpressure, depth=int(depth))
    crddrop_softmax1 = CrdDrop(debug=debug_sim, statistics=report_stats, back_en=backpressure, depth=int(depth))
    crddrop_softmax2 = CrdDrop(debug=debug_sim, statistics=report_stats, back_en=backpressure, depth=int(depth))
    ...
    while not done and time_cnt < TIMEOUT:
        ...
        intersectm3_410.set_in2(fiberlookup_Km_412.out_ref(), fiberlookup_Km_412.out_crd())
