...
@pytest.mark.frostt
def test_tensor4_mult2_ijkml(samBench, frosttname, cast, check_gold, debug_sim, backpressure, depth, report_stats, fill=0):
    B_dirname = os.path.join(formatted_dir, frosttname, "tensor4_mult2_ijkml")
    ...
    V_dirname = os.path.join(formatted_dir, frosttname, "tensor4_mult2_ijkml")
    ...
    fiberlookup_Bj_26 = CompressedCrdRdScan(crd_arr=B_crd1, seg_arr=B_seg1, debug=debug_sim, statistics=report_stats, back_en=backpressure, depth=int(depth))
    fiberlookup_Vj_27 = CompressedCrdRdScan(crd_arr=V_crd2, seg_arr=V_seg2, debug=debug_sim, statistics=report_stats, back_en=backpressure, depth=int(depth))
    intersectj_25 = Intersect2(debug=debug_sim, statistics=report_stats, back_en=backpressure, depth=int(depth))
    fiberlookup_Bk_24 = CompressedCrdRdScan(crd_arr=B_crd2, seg_arr=B_seg2, debug=debug_sim, statistics=report_stats, back_en=backpressure, depth=int(depth))
    repsiggen_k_22 = RepeatSigGen(debug=debug_sim, statistics=report_stats, back_en=backpressure, depth=int(depth))
    repeat_Vk_21 = Repeat(debug=debug_sim, statistics=report_stats, back_en=backpressure, depth=int(depth))
    ...
    while not done and time_cnt < TIMEOUT:
        ...
        fiberlookup_Bj_26.set_in_ref(intersecti_28.out_ref1())
        fiberlookup_Vj_27.set_in_ref(intersecti_28.out_ref2())
        intersectj_25.set_in1(fiberlookup_Bj_26.out_ref(), fiberlookup_Bj_26.out_crd())
        intersectj_25.set_in2(fiberlookup_Vj_27.out_ref(), fiberlookup_Vj_27.out_crd())
        fiberlookup_Bk_24.set_in_ref(intersectj_25.out_ref1())
        repsiggen_k_22.set_istream(fiberlookup_Bk_24.out_crd())
        repeat_Vk_21.set_in_ref(intersectj_25.out_ref2())
        repeat_Vk_21.set_in_repsig(repsiggen_k_22.out_repsig())
        fiberlookup_Vm_20.set_in_ref(repeat_Vk_21.out_ref())
        fiberlookup_Vl_16.set_in_ref(fiberlookup_Vm_20.out_ref())
        repsiggen_m_18.set_istream(fiberlookup_Vm_20.out_crd())
        repeat_Bm_17.set_in_ref(fiberlookup_Bk_24.out_ref())
        ...
        crddrop_12.set_outer_crd(fiberlookup_Bk_24.out_crd())
    ...
    sample_dict = fiberlookup_Bj_26.return_statistics()
    for k in sample_dict.keys():
        extra_info["fiberlookup_Bj_26" + "_" + k] = sample_dict[k]
    ...
    sample_dict = fiberlookup_Bk_24.return_statistics()
    for k in sample_dict.keys():
        extra_info["fiberlookup_Bk_24" + "_" + k] = sample_dict[k]
    ...
    sample_dict = repeat_Vk_21.return_statistics()
    for k in sample_dict.keys():
        extra_info["repeat_Vk_21" + "_" + k] = sample_dict[k]
    ...
    sample_dict = fiberlookup_Vj_27.return_statistics()
    for k in sample_dict.keys():
        extra_info["fiberlookup_Vj_27" + "_" + k] = sample_dict[k]
    ...
    if check_gold:
        print("Checking gold...")
        check_gold_tensor4_mult2_ijkml(frosttname, debug_sim, cast, out_crds, out_segs, out_vals, "ssss0213")
