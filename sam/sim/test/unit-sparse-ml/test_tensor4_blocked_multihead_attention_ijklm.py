...
from sam.sim.src.unary_alu import ScalarMult, Max, Exp, Softmax
...
@pytest.mark.frostt
def test_tensor4_multihead_attention_ijklm(samBench, frosttname, cast, check_gold, debug_sim, backpressure, depth, report_stats, fill=0, block_size=2):
    test_name = "tensor4_fused_mul_T1_blocked"
    ...
    arrayvals_Q_47 = Array(init_arr=Q_vals, block_size=block_size, debug=debug_sim, statistics=report_stats, back_en=backpressure, depth=int(depth))
    arrayvals_K_48 = Array(init_arr=K_vals, block_size=block_size, debug=debug_sim, statistics=report_stats, back_en=backpressure, depth=int(depth))
    arrayvals_V_17 = Array(init_arr=V_vals, block_size=block_size, debug=debug_sim, statistics=report_stats, back_en=backpressure, depth=int(depth))
    repsiggen_m_20 = RepeatSigGen(debug=debug_sim, statistics=report_stats, back_en=backpressure, depth=int(depth))
    mul_46 = Multiply2(debug=debug_sim, delay=2*block_size+1, block_size=block_size, statistics=report_stats, back_en=backpressure, depth=int(depth))
    ...
    mul_15 = Multiply2(debug=debug_sim, delay=2*block_size+1, block_size=block_size, statistics=report_stats, back_en=backpressure, depth=int(depth))
    ...
    fiberwrite_Xvals_0 = ValsWrScan(size=1 * Q_shape[0] * Q_shape[2] * Q_shape[1] * K_shape[1], block_size=block_size, fill=fill, debug=debug_sim, statistics=report_stats, back_en=backpressure, depth=int(depth))
    ...
    scalar_mul = ScalarMult(in2 = 1.0 / sqrt(Q_shape[3]), delay=block_size*block_size, block_size=block_size, debug=debug_sim, statistics=report_stats, back_en=backpressure, depth=int(depth))
    ...
    while not done and time_cnt < TIMEOUT:
        ...
        intersecti_424.set_in1(fiberlookup_Qi_425.out_ref(), fiberlookup_Qi_425.out_crd())
        intersecti_424.set_in2(fiberlookup_Ki_426.out_ref(), fiberlookup_Ki_426.out_crd())

        intersecti2_424.set_in1(fiberlookup_Vi_35.out_ref(), fiberlookup_Vi_35.out_crd())
        intersecti2_424.set_in2(intersecti_424.out_ref2(), intersecti_424.out_crd())

        fiberlookup_Qj_422.set_in_ref(intersecti_424.out_ref1())
        fiberlookup_Kj_423.set_in_ref(intersecti_424.out_ref2())
        fiberlookup_Vj_32.set_in_ref(intersecti2_424.out_ref1())

        intersectj_421.set_in1(fiberlookup_Qj_422.out_ref(), fiberlookup_Qj_422.out_crd())
        intersectj_421.set_in2(fiberlookup_Kj_423.out_ref(), fiberlookup_Kj_423.out_crd())

        intersectj2_421.set_in1(fiberlookup_Vj_32.out_ref(), fiberlookup_Vj_32.out_crd())
        intersectj2_421.set_in2(intersectj_421.out_ref2(), intersectj_421.out_crd())

        fiberlookup_Qk_420.set_in_ref(intersectj_421.out_ref1())
        repsiggen_k_418.set_istream(fiberlookup_Qk_420.out_crd())
        repeat_Vk_26.set_in_ref(intersectj2_421.out_ref1())
        repeat_Vk_26.set_in_repsig(repsiggen_k_418.out_repsig())
        repeat_Kk_417.set_in_ref(intersectj_421.out_ref2())
        ...
        arrayvals_V_17.set_load(fiberlookup_Vm_22.out_ref())

        repsiggen_m_20.set_istream(fiberlookup_Vm_22.out_crd())
        repeat_QKm_19.set_in_repsig(repsiggen_m_20.out_repsig())
        repeat_QKm_19.set_in_ref(softmax.out_val())
