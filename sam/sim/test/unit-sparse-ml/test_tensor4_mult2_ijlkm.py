...
@pytest.mark.frostt
def test_tensor4_mult2_ijlkm(samBench, frosttname, cast, check_gold, debug_sim, report_stats, fill=0):
    B_dirname = os.path.join(formatted_dir, frosttname, "tensor4_mult2_ijlkm")
    ...
    V_dirname = os.path.join(formatted_dir, frosttname, "tensor4_mult2_ijlkm")
    ...
    fiberlookup_Bk_19 = CompressedCrdRdScan(crd_arr=B_crd2, seg_arr=B_seg2, debug=debug_sim, statistics=report_stats)
    ...
    repeat_Vk_16 = Repeat(debug=debug_sim, statistics=report_stats)
    fiberlookup_Vm_15 = CompressedCrdRdScan(crd_arr=V_crd3, seg_arr=V_seg3, debug=debug_sim, statistics=report_stats)
    ...
    spaccumulator2_5 = SpAcc2(debug=debug_sim, val_stkn=True, statistics=report_stats)
    ...
    fiberwrite_Xvals_0 = ValsWrScan(size=1 * B_shape[0] * B_shape[1] * B_shape[2] * V_shape[3], fill=fill, debug=debug_sim, statistics=report_stats)
    fiberwrite_X3_1 = CompressWrScan(seg_size=B_shape[0] * B_shape[1] * B_shape[2] + 1, size=B_shape[0] * B_shape[1] * B_shape[2] * V_shape[3], fill=fill, debug=debug_sim, statistics=report_stats)
    fiberwrite_X1_2 = CompressWrScan(seg_size=B_shape[0] * B_shape[1] + 1, size=B_shape[0] * B_shape[1] * B_shape[2], fill=fill, debug=debug_sim, statistics=report_stats)
    in_ref_B = [0, 'D']
    in_ref_V = [0, 'D']
    done = False
    time_cnt = 0
    ...
    while not done and time_cnt < TIMEOUT:
        ...
        fiberlookup_Bk_19.set_in_ref(intersectl_20.out_ref1())
        repsiggen_k_17.set_istream(fiberlookup_Bk_19.out_crd())
        repeat_Vk_16.set_in_ref(intersectl_20.out_ref2())
        repeat_Vk_16.set_in_repsig(repsiggen_k_17.out_repsig())
        fiberlookup_Vm_15.set_in_ref(repeat_Vk_16.out_ref())
        arrayvals_V_9.set_load(fiberlookup_Vm_15.out_ref())
        repsiggen_m_13.set_istream(fiberlookup_Vm_15.out_crd())
        repeat_Bm_12.set_in_ref(fiberlookup_Bk_19.out_ref())
        repeat_Bm_12.set_in_repsig(repsiggen_m_13.out_repsig())
        arrayvals_B_8.set_load(repeat_Bm_12.out_ref())
        mul_7.set_in1(arrayvals_B_8.out_val())
        mul_7.set_in2(arrayvals_V_9.out_val())
        reduce_6.set_in_val(mul_7.out_val())
        spaccumulator2_5.set_in_crd0(fiberlookup_Vm_15.out_crd())
        spaccumulator2_5.set_in_crd1(fiberlookup_Bk_19.out_crd())
        ...
        fiberwrite_X0_4.set_input(intersecti_26.out_crd())
        fiberwrite_X2_3.set_input(intersectj_23.out_crd())
        fiberwrite_Xvals_0.set_input(spaccumulator2_5.out_val())
        test.append(spaccumulator2_5.out_val())
        # print("val:", remove_emptystr(test))
        fiberwrite_X3_1.set_input(spaccumulator2_5.out_crd1())
        fiberwrite_X1_2.set_input(spaccumulator2_5.out_crd0())
        ...
        crddrop_11.update()
        fiberlookup_Bk_19.update()
        repsiggen_k_17.update()
        repeat_Vk_16.update()
        fiberlookup_Vm_15.update()
    ...
    sample_dict = fiberlookup_Bk_19.return_statistics()
    for k in sample_dict.keys():
        extra_info["fiberlookup_Bk_19" + "_" + k] = sample_dict[k]

    sample_dict = repeat_Vk_16.return_statistics()
    for k in sample_dict.keys():
        extra_info["repeat_Vk_16" + "_" + k] = sample_dict[k]

    sample_dict = fiberlookup_Vm_15.return_statistics()
    for k in sample_dict.keys():
        extra_info["fiberlookup_Vm_15" + "_" + k] = sample_dict[k]
    ...
    if check_gold:
        print("Checking gold...")
        check_gold_tensor4_mult2_ijlkm(frosttname, debug_sim, cast, out_crds, out_segs, out_vals, "ssss0213")
