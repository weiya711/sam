...
@pytest.mark.frostt
def test_tensor3_pos_encoder_mult(samBench, frosttname, cast, check_gold, debug_sim, report_stats, fill=0):
    B_dirname = os.path.join(formatted_dir, frosttname, "tensor3_pos_encoder_mult")
    ...
    scalar = 5
    scal_mul_8 = ScalarMult(in2=scalar, debug=debug_sim)
    in_ref_B = [0, 'D']
    done = False
    time_cnt = 0
    ...
    while not done and time_cnt < TIMEOUT:
        ...
        scal_mul_8.set_in1(arrayvals_B_4.out_val())
        res.append(scal_mul_8.out_val())
        fiberwrite_Xvals_0.set_input(scal_mul_8.out_val())
    ...
    if check_gold:
        print("Checking gold...")
        check_gold_tensor3_pos_encoder_mult(frosttname, debug_sim, cast, out_crds, out_segs, out_vals, scalar, "sss012")
