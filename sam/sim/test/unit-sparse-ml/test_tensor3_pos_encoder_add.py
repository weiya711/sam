...
@pytest.mark.frostt
def test_tensor3_pos_encoder_add(samBench, frosttname, cast, check_gold, debug_sim, report_stats, fill=0):
    B_dirname = os.path.join(formatted_dir, frosttname, "tensor3_pos_encoder_add")
    ...
    C_dirname = os.path.join(formatted_dir, frosttname, "tensor3_pos_encoder_add")
    ...
    repsiggen_i_14 = RepeatSigGen(debug=debug_sim, statistics=report_stats)
    repeat_Ci_13 = Repeat(debug=debug_sim, statistics=report_stats)
    fiberlookup_Cj_12 = CompressedCrdRdScan(crd_arr=C_crd0, seg_arr=C_seg0, debug=debug_sim, statistics=report_stats)
    unionj_10 = Union2(debug=debug_sim, statistics=report_stats)
    ...
    fiberlookup_Ck_9 = CompressedCrdRdScan(crd_arr=C_crd1, seg_arr=C_seg1, debug=debug_sim, statistics=report_stats)
    unionk_7 = Union2(debug=debug_sim, statistics=report_stats)
    ...
    while not done and time_cnt < TIMEOUT:
        ...
        fiberlookup_Bj_11.set_in_ref(fiberlookup_Bi_16.out_ref())
        fiberwrite_X0_3.set_input(fiberlookup_Bi_16.out_crd())
        repsiggen_i_14.set_istream(fiberlookup_Bi_16.out_crd())
        if len(in_ref_C) > 0:
            repeat_Ci_13.set_in_ref(in_ref_C.pop(0))
        repeat_Ci_13.set_in_repsig(repsiggen_i_14.out_repsig())
        fiberlookup_Cj_12.set_in_ref(repeat_Ci_13.out_ref())
        unionj_10.set_in1(fiberlookup_Cj_12.out_ref(), fiberlookup_Cj_12.out_crd())
        unionj_10.set_in2(fiberlookup_Bj_11.out_ref(), fiberlookup_Bj_11.out_crd())
        fiberwrite_X1_2.set_input(unionj_10.out_crd())
        fiberlookup_Bk_8.set_in_ref(unionj_10.out_ref2())
        fiberlookup_Ck_9.set_in_ref(unionj_10.out_ref1())
    ...
    sample_dict = repeat_Ci_13.return_statistics()
    for k in sample_dict.keys():
        extra_info["repeat_Ci_13" + "_" + k] = sample_dict[k]
    ...
    if check_gold:
        print("Checking gold...")
        check_gold_tensor3_pos_encoder_add(frosttname, debug_sim, cast, out_crds, out_segs, out_vals, "sss012")
