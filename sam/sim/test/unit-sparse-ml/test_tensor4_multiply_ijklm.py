...
@pytest.mark.frostt
def test_tensor4_multiply_ijklm(samBench, frosttname, cast, check_gold, debug_sim, backpressure, depth, report_stats, fill=0):
    ...
    fiberlookup_Qi_25 = CompressedCrdRdScan(crd_arr=Q_crd0, seg_arr=Q_seg0, debug=debug_sim, statistics=report_stats, back_en=backpressure, depth=int(depth))
    fiberlookup_Ki_26 = CompressedCrdRdScan(crd_arr=K_crd0, seg_arr=K_seg0, debug=debug_sim, statistics=report_stats, back_en=backpressure, depth=int(depth))
    intersecti_24 = Intersect2(debug=debug_sim, statistics=report_stats, back_en=backpressure, depth=int(depth))
    fiberlookup_Qj_22 = CompressedCrdRdScan(crd_arr=Q_crd2, seg_arr=Q_seg2, debug=debug_sim, statistics=report_stats, back_en=backpressure, depth=int(depth))
    fiberlookup_Kj_23 = CompressedCrdRdScan(crd_arr=K_crd2, seg_arr=K_seg2, debug=debug_sim, statistics=report_stats, back_en=backpressure, depth=int(depth))
    intersectj_21 = Intersect2(debug=debug_sim, statistics=report_stats, back_en=backpressure, depth=int(depth))
    crddrop_9 = CrdDrop(debug=debug_sim, statistics=report_stats, back_en=backpressure, depth=int(depth))
    fiberlookup_Qk_20 = CompressedCrdRdScan(crd_arr=Q_crd1, seg_arr=Q_seg1, debug=debug_sim, statistics=report_stats, back_en=backpressure, depth=int(depth))
    ...
    fiberwrite_X1_3 = CompressWrScan(seg_size=Q_shape[0] + 1, size=Q_shape[0] * Q_shape[2], fill=fill, debug=debug_sim, statistics=report_stats, back_en=backpressure, depth=int(depth))
    ...
    repeat_Kk_17 = Repeat(debug=debug_sim, statistics=report_stats, back_en=backpressure, depth=int(depth))
    fiberlookup_Kl_16 = CompressedCrdRdScan(crd_arr=K_crd1, seg_arr=K_seg1, debug=debug_sim, statistics=report_stats, back_en=backpressure, depth=int(depth))
    fiberlookup_Km_12 = CompressedCrdRdScan(crd_arr=K_crd3, seg_arr=K_seg3, debug=debug_sim, statistics=report_stats, back_en=backpressure, depth=int(depth))
    fiberwrite_X3_1 = CompressWrScan(seg_size=Q_shape[0] * Q_shape[2] * Q_shape[1] + 1, size=Q_shape[0] * Q_shape[2] * Q_shape[1] * K_shape[1], fill=fill, debug=debug_sim, statistics=report_stats, back_en=backpressure, depth=int(depth))
    repsiggen_l_14 = RepeatSigGen(debug=debug_sim, statistics=report_stats, back_en=backpressure, depth=int(depth))
    repeat_Ql_13 = Repeat(debug=debug_sim, statistics=report_stats, back_en=backpressure, depth=int(depth))
    fiberlookup_Qm_11 = CompressedCrdRdScan(crd_arr=Q_crd3, seg_arr=Q_seg3, debug=debug_sim, statistics=report_stats, back_en=backpressure, depth=int(depth))
    intersectm_10 = Intersect2(debug=debug_sim, statistics=report_stats, back_en=backpressure, depth=int(depth))
    arrayvals_Q_7 = Array(init_arr=Q_vals, debug=debug_sim, statistics=report_stats, back_en=backpressure, depth=int(depth))
    arrayvals_K_8 = Array(init_arr=K_vals, debug=debug_sim, statistics=report_stats, back_en=backpressure, depth=int(depth))
    mul_6 = Multiply2(debug=debug_sim, statistics=report_stats, back_en=backpressure, depth=int(depth))
    reduce_5 = Reduce(debug=debug_sim, statistics=report_stats, back_en=backpressure, depth=int(depth))
    ...
    repeat_Ql1_12 = Repeat(debug=debug_sim, statistics=report_stats)
    ...
    while not done and time_cnt < TIMEOUT:
        if len(in_ref_Q) > 0:
            fiberlookup_Qi_25.set_in_ref(in_ref_Q.pop(0))
        if len(in_ref_K) > 0:
            fiberlookup_Ki_26.set_in_ref(in_ref_K.pop(0))
        intersecti_24.set_in1(fiberlookup_Qi_25.out_ref(), fiberlookup_Qi_25.out_crd())
        intersecti_24.set_in2(fiberlookup_Ki_26.out_ref(), fiberlookup_Ki_26.out_crd())
        fiberlookup_Qj_22.set_in_ref(intersecti_24.out_ref1())
        fiberlookup_Kj_23.set_in_ref(intersecti_24.out_ref2())
        ...
        fiberlookup_Qk_20.set_in_ref(intersectj_21.out_ref1())
        fiberwrite_X2_2.set_input(fiberlookup_Qk_20.out_crd())
        repsiggen_k_18.set_istream(fiberlookup_Qk_20.out_crd())
        repeat_Kk_17.set_in_ref(intersectj_21.out_ref2())
        repeat_Kk_17.set_in_repsig(repsiggen_k_18.out_repsig())
        fiberlookup_Kl_16.set_in_ref(repeat_Kk_17.out_ref())
        fiberlookup_Km_12.set_in_ref(fiberlookup_Kl_16.out_ref())
        fiberwrite_X3_1.set_input(fiberlookup_Kl_16.out_crd())
        repsiggen_l_14.set_istream(fiberlookup_Kl_16.out_crd())
        repeat_Ql_13.set_in_ref(fiberlookup_Qk_20.out_ref())
        repeat_Ql_13.set_in_repsig(repsiggen_l_14.out_repsig())
        fiberlookup_Qm_11.set_in_ref(repeat_Ql_13.out_ref())
        intersectm_10.set_in1(fiberlookup_Qm_11.out_ref(), fiberlookup_Qm_11.out_crd())
        intersectm_10.set_in2(fiberlookup_Km_12.out_ref(), fiberlookup_Km_12.out_crd())
        arrayvals_Q_7.set_load(intersectm_10.out_ref1())
        arrayvals_K_8.set_load(intersectm_10.out_ref2())
        mul_6.set_in1(arrayvals_Q_7.out_val())
        mul_6.set_in2(arrayvals_K_8.out_val())
        reduce_5.set_in_val(mul_6.out_val())

        max_reduce_5.set_in_val(reduce_5.out_val())
        repsiggen_l1_13.set_istream(fiberlookup_Kl_16.out_ref())
        repeat_Ql1_12.set_in_ref(max_reduce_5.out_val())
        repeat_Ql1_12.set_in_repsig(repsiggen_l1_13.out_repsig())
        add_10.set_in1(reduce_5.out_val())
        add_10.set_in2(repeat_Ql1_12.out_ref())

        exp_1.set_in1(add_10.out_val())
        reduce_6.set_in_val(exp_1.out_val())
        repsiggen_l_13.set_istream(fiberlookup_Kl_16.out_ref())
        repeat_Ql_12.set_in_ref(reduce_6.out_val())
        repeat_Ql_12.set_in_repsig(repsiggen_l_13.out_repsig())
        div_6.set_in1(exp_1.out_val())
        div_6.set_in2(repeat_Ql_12.out_ref())
        ...
        fiberlookup_Qi_25.update()

        fiberlookup_Ki_26.update()
        ...
        crddrop_9.update()
        fiberlookup_Qk_20.update()
        fiberwrite_X2_2.update()
        repsiggen_k_18.update()
        repeat_Kk_17.update()
        fiberlookup_Kl_16.update()
        fiberlookup_Km_12.update()
        fiberwrite_X3_1.update()
        repsiggen_l_14.update()
        repeat_Ql_13.update()
        fiberlookup_Qm_11.update()
        intersectm_10.update()
        arrayvals_Q_7.update()
        arrayvals_K_8.update()
        mul_6.update()
        reduce_5.update()
        max_reduce_5.update()
        repsiggen_l1_13.update()
        repeat_Ql1_12.update()
        add_10.update()
        exp_1.update()
        reduce_6.update()
    ...
    sample_dict = fiberlookup_Qi_25.return_statistics()
    for k in sample_dict.keys():
        extra_info["fiberlookup_Qi_25" + "_" + k] = sample_dict[k]
    ...
    sample_dict = fiberlookup_Qk_20.return_statistics()
    for k in sample_dict.keys():
        extra_info["fiberlookup_Qk_20" + "_" + k] = sample_dict[k]

    sample_dict = fiberwrite_X2_2.return_statistics()
    for k in sample_dict.keys():
        extra_info["fiberwrite_X2_2" + "_" + k] = sample_dict[k]

    sample_dict = repeat_Kk_17.return_statistics()
    for k in sample_dict.keys():
        extra_info["repeat_Kk_17" + "_" + k] = sample_dict[k]

    sample_dict = fiberlookup_Kl_16.return_statistics()
    for k in sample_dict.keys():
        extra_info["fiberlookup_Kl_16" + "_" + k] = sample_dict[k]

    sample_dict = fiberwrite_X3_1.return_statistics()
    for k in sample_dict.keys():
        extra_info["fiberwrite_X3_1" + "_" + k] = sample_dict[k]

    sample_dict = repeat_Ql_13.return_statistics()
    for k in sample_dict.keys():
        extra_info["repeat_Ql_13" + "_" + k] = sample_dict[k]

    sample_dict = fiberlookup_Qm_11.return_statistics()
    for k in sample_dict.keys():
        extra_info["fiberlookup_Qm_11" + "_" + k] = sample_dict[k]

    sample_dict = intersectm_10.return_statistics()
    for k in sample_dict.keys():
        extra_info["intersectm_10" + "_" + k] = sample_dict[k]

    sample_dict = arrayvals_Q_7.return_statistics()
    for k in sample_dict.keys():
        extra_info["arrayvals_Q_7" + "_" + k] = sample_dict[k]
    ...
    sample_dict = arrayvals_K_8.return_statistics()
    for k in sample_dict.keys():
        extra_info["arrayvals_K_8" + "_" + k] = sample_dict[k]

    sample_dict = fiberlookup_Km_12.return_statistics()
    for k in sample_dict.keys():
        extra_info["fiberlookup_Km_12" + "_" + k] = sample_dict[k]

    sample_dict = fiberlookup_Kj_23.return_statistics()
    for k in sample_dict.keys():
        extra_info["fiberlookup_Kj_23" + "_" + k] = sample_dict[k]

    sample_dict = fiberlookup_Ki_26.return_statistics()
    for k in sample_dict.keys():
        extra_info["fiberlookup_Ki_26" + "_" + k] = sample_dict[k]

    if check_gold:
        print("Checking gold...")
        check_gold_tensor4_multiply_ijklm(frosttname, debug_sim, cast, out_crds, out_segs, out_vals, "ssss0123")
