...
def test_matmul_ijk_spmm(samBench, ssname, cast, check_gold, debug_sim, report_stats, fill=0):
    B_dirname = os.path.join(formatted_dir, ssname, "matmul_ijk_spmm")
    ...
    C_dirname = os.path.join(formatted_dir, ssname, "matmul_ijk_spmm")
    ...
    fiberlookup_Bi_17 = UncompressCrdRdScan(dim=B_shape[0], debug=debug_sim, statistics=report_stats)
    ...
    fiberlookup_Bk_8 = UncompressCrdRdScan(dim=B_shape[1], debug=debug_sim, statistics=report_stats)
