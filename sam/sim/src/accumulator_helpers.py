...
class SpAcc1NoCrdHold(Primitive):
    def __init__(self, maxdim=100, valtype=float, last_level=True, val_stkn=False, depth=1, fifos=None, **kwargs):
        ...
        if fifos is not None:
            self.in_outer_crdpt = fifos[0]
            self.in_inner_crdpt = fifos[1]
            self.in_val = fifos[2]
    ...
    def return_fifo(self):
        return self.in_outer_crdpt, self.in_inner_crdpt, self.in_val
    ...
    def set_in_crd0(self, crdpt, parent=None):
    ...
    def set_in_crd1(self, crdpt, parent=None):
        assert not is_stkn(crdpt), 'Coordinate points should not have stop tokens'
        if crdpt != '' and crdpt is not None:
...
class SpAcc2NoCrdHold(Primitive):
    ...
    def set_in_crd2(self, crdpt, parent=None):
        assert not is_stkn(crdpt), 'Coordinate points should not have stop tokens'
        if crdpt != '' and crdpt is not None:
    ...
    def update(self):
        ...
        if self.debug:
            if self.seen_done or self.done:
                ...
                print("SparseAccumulatorNoCrdHold1", self.outer_crdpt, self.inner_crdpt, self.in_val, self.emit_output,
