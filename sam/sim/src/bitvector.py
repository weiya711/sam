    ...
    def update(self):
        ...
        if self.done:
            ...
            if isinstance(in_crd, int):
                self.curr_bv = ''
                if in_crd >= self.meta_width * self.count:
                    self.curr_bv = 0 if self.crds is None else reduce(lambda a, b: a | b, [0b1 << c for c in self.crds])
                    self.count += 1
                    self.emit_more_zeros = in_crd >= self.meta_width * self.count
                self.crds.append(in_crd)
            elif is_stkn(in_crd):
                self.stkn = in_crd
                if self.meta_width * self.count < self.meta_size
