    ...
    def update(self):
        ...
        if (self.backpressure_en and self.check_backpressure()) or not self.backpressure_en:
            ...
            for j in range(self.sf):
                ...
                self.rd_scanners[j].update()
            for i in range(self.layers_num):
                for j in range(len(self.union_tree[i])):
                    if i == 0:
                        self.union_tree[i][j].set_in1(self.rd_scanners[2 * j].out_ref(),
                                                      self.rd_scanners[2 * j].out_crd(),
                                                      self.rd_scanners[2 * j].return_upper_crd())
                        self.union_tree[i][j].set_in2(self.rd_scanners[2 * j + 1].out_ref(),
                                                      self.rd_scanners[2 * j + 1].out_crd(),
                                                      self.rd_scanners[2 * j + 1].return_upper_crd())
                        self.union_tree[i][j].update()
                    else:
                        self.union_tree[i][j].set_in1(self.union_tree[i - 1][2 * j].out_ref_min(),
                                                      self.union_tree[i - 1][2 * j].out_crd(),
                                                      self.union_tree[i - 1][2 * j].out_ocrd_i())
                        self.union_tree[i][j].set_in2(self.union_tree[i - 1][2 * j + 1].out_ref_min(),
                                                      self.union_tree[i - 1][2 * j + 1].out_crd(),
                                                      self.union_tree[i - 1][2 * j + 1].out_ocrd_i())
                        self.union_tree[i][j].update()
