...
#generating matrix dimensions and storing results in an array, array size is 20, 10 matrices and 2 dimensions per matrix
...
# dimensions = [10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,10]
dimensions = [30,30,30,30,30,30,30,30,30,30,30,30,30,30,30,30,30,30,30,30]
...
# for x in range(9):
    ...
    #testing - hardcoded dimensions for one random matrix
...
ss_path = '/nobackup/jadivara/sparse-datasets/matrices'
for i in range(10):
    ...
    f.write('\n')
    lineToAddInFile = ""
    arr = np.zeros([dimensions[i*2],dimensions[(i*2)+1]], dtype=int)
    for x in range(len(arr)):
        for y in range(len(arr[x])):
            #TO CHANGE SPARSITY: generate random number from 1 to 9; if 1,2,3,7,8,9 don't add a num in, only add if 4,5,6
            # randomNumber = random.randint(1,9)
            #updated sparsity control below, based on numbers in sparsity array
    ...
    with open(filename, 'r') as f:
        content = f.read()
    updated_content = ""+str(dimensions[i*2]) + " " + str(dimensions[i*2+1]) + " " + str(countnnz) + content
