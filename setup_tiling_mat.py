...
from sam.util import FROSTT_PATH
# print('frostt_path is: ', FROSTT_PATH)
...
for datum in data:
   # print(FROSTT_PATH)
   tns_file = glob.glob(f"{FROSTT_PATH}/{datum}.tns")[0]
   breakpoint()
   # mtx_file = glob.glob(f"{SUITESPARSE_PATH}/{datum}.mtx")[0]
   os.makedirs("extensor_tns", exist_ok=True)
   shutil.copy(tns_file,f"extensor_tns/{datum}.tns")
