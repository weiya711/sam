...
def plot_stream_overhead(input_csv, outfile):
    ...
    with open(input_csv) as csvfile: 
        ...
        for row in reader:
            ...
            outer_total = 1 + int(row['stream_Bi_empty']) + int(row['stream_Bi_noncontrol']) + int(row['stream_Bi_stop'])
            ...
            # Only ever 1 done token at the end of a stream
            inner_total = 1 + int(row['stream_Bj_empty']) + int(row['stream_Bj_noncontrol']) + int(row['stream_Bj_stop'])
        ...
        dims, mtx_names, outer_stream_done, outer_stream_idle, outer_stream_nonctrl, outer_stream_stop, inner_stream_done, inner_stream_idle, inner_stream_nonctrl, inner_stream_stop = zip(*sorted(zip(dims, mtx_names, outer_stream_done, outer_stream_idle, outer_stream_nonctrl, outer_stream_stop, inner_stream_done, inner_stream_idle, inner_stream_nonctrl, inner_stream_stop)))
        ...
        ax1.bar(mtx_names, outer_stream_done, bottom=[outer_stream_nonctrl[i] + outer_stream_stop[i] for i in range(nrows)], color='y')
        ax1.bar(mtx_names, outer_stream_idle, bottom=[outer_stream_nonctrl[i] + outer_stream_stop[i] + outer_stream_done[i] for i in range(nrows)], color='g')
        ax1.set_xticklabels([])

        ax2.bar(mtx_names, inner_stream_nonctrl, color='b')
        ax2.bar(mtx_names, inner_stream_stop, bottom=inner_stream_nonctrl, color='r')
        ax2.bar(mtx_names, inner_stream_done, bottom=[inner_stream_nonctrl[i] + inner_stream_stop[i] for i in range(nrows)], color='y')
        ax2.bar(mtx_names, inner_stream_idle, bottom=[inner_stream_nonctrl[i] + inner_stream_stop[i] + inner_stream_done[i] for i in range(nrows)], color='g')
        ax2.set_xticklabels(mtx_names, fontsize=5, rotation=90)

        # ax1.set_xticklabels(mtx_names, fontsize=5, rotation=90)
