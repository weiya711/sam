...
from scripts.util.util import NumpyNPYArrayLoader
import argparse
import os
import shutil
import scipy.sparse
...
def write_datastructure_bench(args, tensor, out_path, tiles=None):
    ...
    coo = inputCache.load(tensor, False)
    shape = coo.shape
    formatWriter.writeout_separate_sparse_only(coo, dirname, args.name, format_str=args.format)
...
inputCache = InputCacheSuiteSparse(NumpyNPYArrayLoader())
formatWriter = FormatWriter(args.cast)

cwd = os.getcwd()
if args.output_dir_path is None:
    out_dirname = os.getenv("SPARSE_ML_FORMATTED_PATH", default=os.path.join(cwd, 'sparse-ml-formatted'))
else:
...
if args.input_path is None:
    SPML_PATH = os.getenv('SPARSE_ML_TENSOR_PATH', default=os.path.join(cwd, 'suitesparse'))

else:
    SPML_PATH = args.input_path
...
if args.tiles:
    ...
    print(SPML_PATH)
    tensor = SuiteSparseTensor(SPML_PATH)
