...
for nnz in ${!NNZ[@]}; do
	for dim in ${!DIMENSIONS[@]}; do			
		...
		for b in ${!BENCHMARKS[@]}; do
			...
			cd $basedir/sam/sim
			if [ $2 -eq 1 ]; then
				pytest test/advanced-simulator/test_$bench_name.py --ssname $line -s --check-gold --skip-empty --nbuffer --yaml_name=$1 --nnz-value=${NNZ[$nnz]} --benchmark-json=$path/${line}_${NNZ[$nnz]}_${DIMENSIONS[$dim]}.json 
			else
				pytest test/advanced-simulator/test_$bench_name.py --ssname $line -s --skip-empty --nbuffer --yaml_name=$1 --nnz-value=${NNZ[$nnz]} --benchmark-json=$path/${line}_${NNZ[$nnz]}_${DIMENSIONS[$dim]}.json 
			fi
