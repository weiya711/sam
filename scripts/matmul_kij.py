...
def test_matmul_kij(ssname, debug_sim, fill = 0):
    C_dirname = os.path.join(formatted_dir, ssname, "orig", "ds01")
    ...
    B_dirname = os.path.join(formatted_dir, ssname, "shift-trans", "ss10")
    ...
    fiberlookup_Bk = CompressedRdScan(crd_arr=B_crd0, seg_arr=B_seg0, debug=debug_sim)
    intersect = Intersect2(debug = debug_sim)
    fiberlookup_Bi = CompressedRdScan(crd_arr=B_crd1, seg_arr=B_seg1, debug=debug_sim)
    ...
    fiberwrite_X1 = CompressWrScan(seg_size = B_shape[1] + 1, size=B_shape[1] * C_shape[1], fill = fill, debug = debug_sim)
    arrayvals_C = Array(init_arr= C_vals, debug = debug_sim)
    mul = Multiply2(debug=debug_sim)
    fiberwrite_Xvals = ValsWrScan(size= 1 * B_shape[1] * C_shape[1], fill=fill, debug=debug_sim)
    fiberwrite_X0 = CompressWrScan(seg_size = 2, size=B_shape[1], fill = fill, debug = debug_sim)
    in_ref_C = [0, 'D']
    in_ref_B = [0, 'D']
    done = False
    time = 0
    ...
    while not done and time < TIMEOUT:
        if len(in_ref_B) > 0:
            fiberlookup_Bk.set_in_ref(in_ref_B.pop(0))
        fiberlookup_Bk.update()

        if len(in_ref_C) > 0:
            fiberlookup_Ck.set_in_ref(in_ref_C.pop(0))
        fiberlookup_Ck.update()

        fiberlookup_Bi.set_in_ref(intersect.out_ref_out-B())
        ...
        repeat_Bj.set_in_ref(fiberlookup_Bi.out_ref())
        ...
        repeat_Ci.set_in_ref_out-C(intersect.out_ref_out-C())
