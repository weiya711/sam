...
def test_matmul_kji(ssname, debug_sim, fill = 0):
    ...
    fiberwrite_X1 = CompressWrScan(seg_size = C_shape[1] + 1, size=C_shape[1] * B_shape[1], fill = fill, debug = debug_sim)
    arrayvals_B = Array(init_arr= B_vals, debug = debug_sim)
    mul = Multiply2(debug=debug_sim)
    fiberwrite_Xvals = ValsWrScan(size= 1 * C_shape[1] * B_shape[1], fill=fill, debug=debug_sim)
    fiberwrite_X0 = CompressWrScan(seg_size = 2, size=C_shape[1], fill = fill, debug = debug_sim)
    in_ref_C = [0, 'D']
    in_ref_B = [0, 'D']
    done = False
    time = 0
    ...
    while not done and time < TIMEOUT:
        ...
        arrayvals_C.set_load(repeat_Ci.out_ref())
        arrayvals_C.update()

        mul.set_in1(arrayvals_C.out_load())
        mul.set_in2(arrayvals_B.out_load())
        mul.update()
